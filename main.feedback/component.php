...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] <> '' && (!isset($_POST["PARAMS_HASH"]) || $arResult["PARAMS_HASH"] === $_POST["PARAMS_HASH"]))
{
	...
	if(check_bitrix_sessid())
	{
		if(empty($arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $arParams["REQUIRED_FIELDS"]))
		{
			...
			foreach($arParams['NEW_EXT_FIELDS'] as $i => $fields){
				if(in_array("custom_$i", $arParams["REQUIRED_FIELDS"]) && (!$_POST['custom'] || !array_key_exists($i, $_POST['custom']) || strlen($_POST['custom'][$i]) <= 1)){
					$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_EXT_FIELDS").$fields;
				}
			}
